...
- **Авто-пара (тестовый режим):** свежая установка A-app на любой телефон сразу
  появляется в списке устройств B-app — тестовая пара вшита в сборку
  (`AUTO_PAIR_ID`/`AUTO_PAIR_SECRET` в `android/grandma/build.gradle.kts`), QR не нужен.
  QR-спаривание остаётся запасным путём (чек-лист «Aktivering» → шаг pairing).
